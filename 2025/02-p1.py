...
# Find the ranges in the txt files 
# for numbers with even number of digits
#   if number first half == number second half
#       add number to ID sum

def is_invalid(i):

    i_str = str(i)
    length = len(i_str)
    first_half = i_str[:int(length/2)]
    second_half = i_str[int(length/2):]
...
for single_range in ranges:
    ...
    for i in range(start, end + 1):
        if len(str(i)) % 2 == 0 and is_invalid(i):
            invalid_ids_sum += i
